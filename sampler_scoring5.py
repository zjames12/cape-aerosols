...
model.load_state_dict(load_file("cape-aerosol-gfs-summer-2000-v8/checkpoints/model_diffusion_cape_gfs_8.1_999.safetensors"))
...
for day in days:
    for letter in ['b', 'c', 'd', 'e']:
        file = "/home/zj37/pfi/scoring-combined-6" + letter + "/" + day.strftime("%Y%m%d") + ".tif"
        if not Path(file).exists():
            continue
        dir = "crs-6/" + (day).strftime("%Y%m%d") + f"/ensemble8.1.100.0.6.{letter}/"
