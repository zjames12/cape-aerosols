...
class TrainingConfig:
    ...
    output_dir = 'cape-aerosol-gefs-summer-2000-v10-no-base'  # the model name locally and on the HF Hub
...
model.load_state_dict(load_file("cape-aerosol-gefs-summer-2000-v10-no-base/temp.safetensors"))
...
def train_loop(config, model, noise_scheduler, optimizer, train_dataloader, lr_scheduler):
    ...
    # Train the model
    for epoch in range(438, 600):#range(config.num_epochs):
        ...
        if accelerator.is_main_process:
            ...
            if (epoch + 1) % config.save_model_epochs == 0 or epoch == config.num_epochs - 1:
                ...
                save_file(model.state_dict(), f"{config.output_dir}/checkpoints/model_diffusion_cape_gfs_10_{epoch}.safetensors")
