...
class TrainingConfig:
    ...
    output_dir = 'cape-no-aerosol-gfs-summer-2000-v8'  # the model name locally and on the HF Hub
...
import datasets
# dataset = datasets.load_from_disk("gfs-merra-1800-2400-summer-forecast-arrow-table")
# dataset = datasets.load_from_disk("gefs-only-merra-1800-2400-summer-finetune-forecast-arrow-table")
...
# model.load_state_dict(load_file("cape-aerosol-gfs-forecast-v4/checkpoints/model_diffusion_cape_gfs_4.0_599.safetensors"))
# model.load_state_dict(load_file("cape-aerosol-gfs-summer-forecast-v6/checkpoints/model_diffusion_cape_gfs_6.0_999.safetensors"))
# model.load_state_dict(load_file("cape-aerosol-gfs-summer-partial-v7/checkpoints/model_diffusion_cape_gfs_7.1_499.safetensors"))
# model.load_state_dict(load_file("cape-aerosol-gefs-summer-2000-v8/checkpoints/model_diffusion_cape_gfs_8.2_399.safetensors"))
model.load_state_dict(load_file("cape-no-aerosol-gfs-summer-2000-v8/checkpoints/model_diffusion_cape_no_aerosol_gfs_8_999.safetensors"))

sample_image = dataset[0]['fields'].unsqueeze(0)
sample_image = sample_image[:,[0,6,7],:,:]
...
def evaluate(config, epoch, pipeline):
    ...
    images = pipeline(
        torch.stack(dataset[:16]['fields'])[:,[0,6],:,:], 
...
def train_loop(config, model, noise_scheduler, optimizer, train_dataloader, lr_scheduler):
    ...
    logging_dir = os.path.join(config.output_dir, "logs.8.2")
    ...
    for epoch in range(1000, 1400):#range(config.num_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            attributes = batch['fields'][:,[0,6],:,:] # :7 <--> [0,6]
        ...
        if accelerator.is_main_process:
            ...
            if (epoch + 1) % config.save_model_epochs == 0 or epoch == config.num_epochs - 1:
                ...
                save_file(model.state_dict(), f"{config.output_dir}/checkpoints/model_diffusion_cape_no_aerosol_gfs_8.2_{epoch}.safetensors")
